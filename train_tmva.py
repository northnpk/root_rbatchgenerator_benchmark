...
def load_dataset(data_path='/home/northnpk/Downloads/JetClass_dataset/',
    ...
    dataloader = DataLoader(dataset=dataset, collate_fn=dataset.collate_fn, batch_size=None)
    # print(f'RBatchGenerator init time count: {timeit.default_timer()-start}')
    return dataloader
...
def train(model, loss_fn, optimizer, train_loader):
    ...
    for (x_part, x_jet, y) in train_loader:
        # print(f'loop data: {x_part.shape}')
        pass
